...
public class DiskBasedCache implements ICache {
    ...
    public synchronized void remove(String key) {
        ...
        if (!deleted) {
            Loger.debug(
                    String.format("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key)));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        while (iterator.hasNext()) {
            ...
            if (deleted) {
                ...
                Loger.debug(String.format("Could not delete cache entry for key=%s, filename=%s", e.key,
        ...
        Loger.debug(String.format("pruned %d files, %d bytes, %d ms", prunedFiles, (mTotalSize - before),
