...
public class NetworkDispatcher extends Thread {
    ...
    public NetworkDispatcher(BlockingQueue<Request<?>> queue, INetwork network, ICache cache, IDelivery delivery) {
        mQueue = queue;
        mNetwork = network;
        mCache = cache;
        mDelivery = delivery;
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                String localResult = request.getCallback().loadLocalTaskForAsync();
                mDelivery.postLocalTaskResult(localResult, request);
                if (request.getCallback().isDropNetRequest()) {
                    request.finish("drop net request!");
                    continue;
                }
                ...
                if (networkResponse.data != null) {
                    ...
                    mPoster.post(new Result(request.getUrl(), networkResponse.headers, networkResponse.data));
                }
