...
import static com.kymjs.rxvolley.http.VolleyError.ErrorConst.ERROR_CODE_206;
...
import static com.kymjs.rxvolley.http.VolleyError.ErrorConst.ERROR_CODE_SERVER_ERROR;
...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    public void deliverError(VolleyError error) {
        if (mCallback != null) {
            int errorNo;
            String strMsg = "未知异常，请稍候重试！";
            if (error != null) {
                errorNo = error.getStatusCode();
                strMsg = error.getMessage();
                Loger.debug(strMsg);
            } else {
                errorNo = ERROR_CODE_UNKNOW;
            }
            switch (errorNo) {
                case ERROR_CODE_UNKNOW:
                    strMsg = "网络异常，请检查您的网络状态！";
                    break;
                case ERROR_CODE_206:
                    strMsg = "网络异常或空间不足，请检查您的网络状态、磁盘空间！";
                    break;
                case ERROR_CODE_NO_CONNECT:
                    strMsg = "网络连接失败，请检查您的网络状态！";
                    break;
