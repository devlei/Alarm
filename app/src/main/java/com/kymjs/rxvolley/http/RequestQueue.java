 ...
 * Note:
 * 整个RequestQueue工作流程：采用责任链设计模式，由三部分组成，类似设计可以类比Handle...Looper...MessageQueue
 * <br>
...
public class RequestQueue {
        ...
        if (request.shouldCache()) {
            synchronized (mWaitingRequests) {
                ...
                if (waitingRequests != null) {
                    Loger.debug(String.format("Releasing %d waiting requests for cacheKey=%s.", waitingRequests.size(),
                            cacheKey));
    ...
    public synchronized static RequestQueue newRequestQueue(File cacheFolder, IHttpStack httpStack) {
