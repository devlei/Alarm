...
import static com.kymjs.rxvolley.http.VolleyError.ErrorConst.ERROR_CODE_BAD_URL;
import static com.kymjs.rxvolley.http.VolleyError.ErrorConst.ERROR_CODE_NO_CONNECT;
import static com.kymjs.rxvolley.http.VolleyError.ErrorConst.ERROR_CODE_TIME_OUT;
...
public class Network implements INetwork {
    ...
    private String isContainHttpsJumpPoint(Request<?> request, URLHttpResponse httpResponse) {
        // Location
        String sourceReqUrl = request.getUrl();
        if (!TextUtils.isEmpty(sourceReqUrl) && null != httpResponse) {
            if (sourceReqUrl.startsWith("http://")) {
                long contentLength = httpResponse.getContentLength();
                if (contentLength == 0) {
                    HashMap<String, String> responseHeaders = httpResponse.getHeaders();
                    if (null != responseHeaders) {
                        String location = responseHeaders.get("Location");
                        Loger.debug("location:" + location);
                        boolean flag = !TextUtils.isEmpty(location) && location.startsWith("https://");
        ...
        while (true) {
            ...
            try {
                ...
                if (request instanceof FileRequest) {
                    String jumpPointUrl = isContainHttpsJumpPoint(request, httpResponse);
                    if (!TextUtils.isEmpty(jumpPointUrl)) {
                        request.getConfig().mUrl = jumpPointUrl;
                        httpResponse = mHttpStack.performRequest(request, headers);
                    }
                }
                ...
                VolleyError volleyError = new VolleyError(new SocketTimeoutException("socket timeout"));
                volleyError.setStatusCode(ERROR_CODE_TIME_OUT);
                attemptRetryOnException("socket", request, volleyError);
            } catch (MalformedURLException e) {
                VolleyError volleyError = new VolleyError("Bad URL " + request.getUrl(), e);
                volleyError.setStatusCode(ERROR_CODE_BAD_URL);
                attemptRetryOnException("connection", request, volleyError);
            } catch (IOException e) {
                int statusCode;
                if (httpResponse != null) {
                    statusCode = httpResponse.getResponseCode();
                } else {
                    VolleyError volleyError = new VolleyError("NoConnection error", e);
                    volleyError.setStatusCode(ERROR_CODE_NO_CONNECT);
                    throw volleyError;
                }
                ...
                if (responseContents != null) {
                    ...
                    VolleyError volleyError = new VolleyError(String.format("Unexpected response code %d for %s", statusCode, request.getUrl()));
                    volleyError.setStatusCode(ERROR_CODE_UNKNOW);
                    throw volleyError;
                }
            }
        }
