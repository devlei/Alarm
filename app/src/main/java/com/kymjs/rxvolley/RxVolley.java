...
public class RxVolley {
    ...
    public synchronized static RequestQueue getRequestQueue() {
        if (sRequestQueue == null) {
            if(!CACHE_FOLDER.exists()){
                sRequestQueue = RequestQueue.newRequestQueue(Environment.getDataDirectory());
            }
            else{
    ...
    public static class Builder {
        ...
        /**
         * 查看RequestConfig$Method GET/POST/PUT/DELETE/HEAD/OPTIONS/TRACE/PATCH
         */
    ...
    public static void post(String url, HttpParams params, ProgressListener listener, HttpCallback callback) {
        new Builder().url(url).params(params).progressListener(listener).httpMethod(Method.POST).callback(callback)
                .doTask();
    }

    public static void jsonGet(String url, HttpParams params, HttpCallback callback) {
        new Builder().url(url).params(params).contentType(ContentType.JSON).httpMethod(Method.GET).callback(callback)
                .doTask();
    }

    public static void jsonPost(String url, HttpParams params, HttpCallback callback) {
        new Builder().url(url).params(params).contentType(ContentType.JSON).httpMethod(Method.POST).callback(callback)
                .doTask();
    }
     ...
     * @param callback 回调
     */
    public static void download(String storeFilePath, String url, ProgressListener progressListener,
